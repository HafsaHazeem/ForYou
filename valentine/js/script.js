...
let slideIndex = 0;
const totalSlides = document.querySelectorAll('.image-viewer .slides img').length;

function showSlide(index) {
    const slides = document.querySelector('.image-viewer .slides');
    if (index < 0) {
    ...
    slides.style.transform = `translateX(${offset}%)`; /* Use translateX for horizontal sliding */
}
...
document.querySelectorAll('.image-viewer .slides img').forEach((img, index) => {
    if (index !== 0) {
